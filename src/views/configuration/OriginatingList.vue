...
import {
  ...
  addOriginatingApi,
  updateOriginatingStatusApi,
  deleteOriginatingDetailApi
} from "@/api/selects"
...
const rules = reactive({
  name: [{ required: true, message: "請輸入起運港名稱", trigger: "blur" }]
})
...
const submitForm = (formRef) => {
  ...
  formRef.validate((valid) => {
    if (valid) {
      addOriginatingApi(formData).then(() => {
...
const { handleDelete, isDeleted } = useDeleteList({
  api: deleteOriginatingDetailApi,
  text: "起運港？"
})
...
<template>
  <div class="app-container">
    <el-card shadow="never" class="search-wrapper">
      <el-input v-model="searchData.keyword" placeholder="輸入起運港名稱" style="width: 300px; margin-right: 15px" />
    ...
    <el-card v-loading="loading" shadow="never">
      <div class="pb5">
        <div class="flex justify-between mb5">
          <el-text tag="b" size="large">起運港管理</el-text>
          <div>
    ...
    <el-dialog v-model="dialogVisible" :title="formDataTitle" width="40%" destroy-on-close>
      <el-form ref="formRef" :model="formData" :rules="rules" hide-required-asterisk>
        <el-form-item label="起運港名稱" prop="name">
