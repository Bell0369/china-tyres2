...
import {
  ...
  addFreightForwarderApi,
  updateFreightForwarderStatusApi,
  deleteFreightForwarderApi
} from "@/api/selects"
...
const rules = reactive({
  name: [{ required: true, message: "請輸入船代名稱", trigger: "blur" }]
})
...
const submitForm = (formRef) => {
  ...
  formRef.validate((valid) => {
    if (valid) {
      addFreightForwarderApi(formData).then(() => {
...
<template>
  <div class="app-container">
    <el-card shadow="never" class="search-wrapper">
      <el-input v-model="searchData.keyword" placeholder="輸入船代名稱" style="width: 300px; margin-right: 15px" />
    ...
    <el-card v-loading="loading" shadow="never">
      <div class="pb5">
        <div class="flex justify-between mb5">
          <el-text tag="b" size="large">船代管理</el-text>
          <div>
    ...
    <!-- 新增/修改 -->
    <el-dialog v-model="dialogVisible" :title="formDataTitle" width="40%" destroy-on-close>
      <el-form ref="formRef" :model="formData" :rules="rules" hide-required-asterisk>
        <el-form-item label="船代名稱" prop="name">
