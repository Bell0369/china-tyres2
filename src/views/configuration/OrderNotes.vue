...
import { getOrderRemarksConfigListApi, addOrderRemarksConfigApi, deleteOrderRemarksConfigApi } from "@/api/selects"
...
const rules = reactive({
  content: [{ required: true, message: "請輸入訂單備註", trigger: "blur" }]
})
...
const submitForm = (formRef) => {
  ...
  formRef.validate((valid) => {
    if (valid) {
      addOrderRemarksConfigApi(formData).then(() => {
...
const { handleDelete, isDeleted } = useDeleteList({
  api: deleteOrderRemarksConfigApi,
  text: "訂單備註？"
})
...
<template>
  <div class="app-container">
    <el-card shadow="never" class="search-wrapper">
      <el-input v-model="searchData.keyword" placeholder="輸入備註內容" style="width: 300px; margin-right: 15px" />
    ...
    <el-dialog v-model="dialogVisible" :title="formDataTitle" width="30%" destroy-on-close>
      <el-form ref="formRef" :model="formData" :rules="rules">
        <el-form-item label="訂單備註" prop="content">
